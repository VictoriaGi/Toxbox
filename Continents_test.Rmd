...
```{r}
med_country_cyanotox <- medcon_clean %>% 
  filter(Continent == "Mediterranean")%>%
  select(Species, Country,Cyanotoxin)%>%
  group_by(Species, Cyanotoxin)%>%
    arrange(Species)
...
ggplot(med_country_cyanotox, aes(Country, y= Species)) + geom_bar(stat = "identity")
...
cont_subset <- head(main_toxin_producers_globally,8)
